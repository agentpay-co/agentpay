...
How AgentPay's pieces fit together today, and where the architecture is headed
per [ROADMAP.md](../ROADMAP.md).

AgentPay is the payment rail AI agents spend through on Stellar. You hand an
...
| `packages/orchestrator` | The core service. Plans tasks with an LLM, validates and executes the plan, talks to AgentVault to lock and release funds, pays agents over x402/MPP, and serves the dashboard over REST and WebSocket. |
...
| `contracts/agent-vault` | AgentVault, the Soroban contract that holds user USDC and enforces the budget lifecycle on-chain. |
| `contracts/budget-guardian` | An earlier budget-tracking contract, superseded by AgentVault and kept for reference. |

## Trust model

AgentPay's trustlessness applies to some layers and not others. Being clear
about this matters for anyone evaluating it.

### Enforced on-chain

- **Custody.** AgentVault holds all user funds. The contract is the only thing
...
AgentVault holds USDC on behalf of users. The orchestrator only touches user
...
AgentVault, an on-chain agent registry, a Stellar MCP server, a reusable agent
