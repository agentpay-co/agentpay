...
enforced on-chain but kept private, the thing that separates AgentPay from
...
- **AgentVault** Soroban contract: deposits, per-task budget locking, per-step
...
into AgentVault is the main build.
...
- An MCP server exposing agent discovery, vault views, and payment helpers as
  MCP tools, so any MCP-compatible client can find and pay AgentPay agents.
- `@agentpay/agent-sdk`: shared scaffolding for specialist agents (x402/MPP
...
- Security review of AgentVault, including the policy-verification path.
...
See the [issue tracker](https://github.com/agentpay-protocol/agentpay/issues)
