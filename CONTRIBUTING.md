...
Thanks for your interest in contributing to AgentPay, a way to delegate a
...
│   ├── agent-vault/        # AgentVault, on-chain treasury for tasks
...
```bash
git clone https://github.com/agentpay-protocol/agentpay.git
cd agentpay
npm install
cp .env.example .env
```
...
- **Commit messages**: this repo uses [Conventional
  ...
  docs: document AgentVault authorization model
...
Priority is on: private spending policies in AgentVault (the headline next
step), hardening the AgentVault contract, an on-chain agent registry, a Stellar
