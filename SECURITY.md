...
AgentPay currently runs on **Stellar Testnet**. Contracts, wallets, and funds
involved are all testnet assets with no real-world value. Even so, we treat
security issues seriously, since the AgentVault contract, its planned spending-
...
- `contracts/agent-vault` (AgentVault) and `contracts/budget-guardian`:
...
- The planned spending-policy layer for AgentVault (proof verification, policy
...
- Third-party services AgentPay depends on (the Stellar network itself, the
