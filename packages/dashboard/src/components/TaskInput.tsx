    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="w-full max-w-sm bg-gray-900 border border-amber-800/50 rounded-2xl shadow-2xl">
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <button
            onClick={onFund}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-sm font-semibold transition-all shadow-lg shadow-teal-900/30"
          >
            <Wallet size={14} />
            Fund Vault
          </button>
    ...
    <>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800/80">
          ...
          >
            <div className="w-8 h-8 bg-gradient-to-br from-teal-600 to-teal-800 rounded-xl flex items-center justify-center shadow-lg shadow-teal-900/40 group-hover:from-teal-500 group-hover:to-teal-700 transition-all">
        ...
        <div className="p-4">
          <p className="text-xs text-gray-500 mb-2.5">
            What do you want <span className="text-teal-400 font-medium">{orchestratorName}</span> to do?
          </p>
          ...
          <div className="relative">
            <textarea
              ...
              rows={2}
              className="w-full bg-gray-800/80 border border-gray-700/80 rounded-xl px-3.5 py-3 pr-11 text-sm text-gray-200 placeholder-gray-600 resize-none focus:outline-none focus:border-teal-500/70 focus:bg-gray-800 transition-all disabled:opacity-50"
            />
            <button
              ...
              className="absolute bottom-2.5 right-2.5 w-7 h-7 flex items-center justify-center bg-teal-600 hover:bg-teal-500 disabled:bg-gray-700 disabled:text-gray-600 text-white rounded-lg transition-all shadow-sm"
            >
              <Send size={12} />
            </button>
          </div>
          ...
          {!task && !isRunning && (
            <div className="mt-2.5 space-y-1.5">
              {EXAMPLE_TASKS.map((t, i) => (
                ...
                >
                  <Sparkles size={9} className="inline mr-1.5 text-teal-500" />
                  {t}
                </button>
              ))}
            </div>
              ...
              className={`w-full bg-gray-800/80 border rounded-xl pl-6 pr-14 py-2 text-sm font-mono font-semibold focus:outline-none transition-all disabled:opacity-50 ${
                !balanceOk && budgetValid
                  ...
                  : !budgetValid && budgetInput !== ''
                    ...
                    : 'border-gray-700/80 text-teal-300 focus:border-teal-500/70'
              }`}
            />
