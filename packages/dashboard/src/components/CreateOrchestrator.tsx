    ...
    try {
      if (!BACKEND_ENABLED) {
        ...
        localStorage.setItem(
          `agentpay_orchestrator_${publicKey}`,
          JSON.stringify({
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center px-4">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-xl font-bold text-white leading-none">AgentPay</h1>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 w-full max-w-md">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-teal-500 disabled:opacity-50"
          />
          {/* Quick-pick names */}
          <div className="flex flex-wrap gap-1.5 mt-2">
            {EXAMPLE_NAMES.map((n) => (
              <button
                ...
                onClick={() => setName(n)}
                disabled={isWorking}
                className="text-xs px-2 py-0.5 rounded-full bg-gray-800 border border-gray-700 text-gray-400 hover:border-teal-600 hover:text-teal-400 transition-colors disabled:opacity-40"
              >
                {n}
              </button>
            ))}
        ...
        <div className="mb-6">
          ...
          {showPrompt && (
            <textarea
              ...
              className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-teal-500 resize-none disabled:opacity-50"
            />
          )}
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-500 disabled:bg-teal-900 disabled:cursor-not-allowed text-white font-medium py-3 px-4 rounded-xl transition-colors text-sm"
        >
          {isWorking ? (
            <>
