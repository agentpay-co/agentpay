...
function ItemEditForm({
  ...
  return (
    <div className="px-3 py-2.5 space-y-2.5 bg-gray-800/60 border border-teal-800/40 rounded-xl">
      <textarea
        rows={2}
        value={task}
        onChange={e => setTask(e.target.value)}
        autoFocus
        className="w-full bg-gray-800 border border-gray-700 focus:border-teal-500/70 rounded-lg px-2.5 py-1.5 text-xs text-gray-200 placeholder-gray-600 resize-none outline-none transition-colors"
      />
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="0.00"
            className="w-full bg-gray-800 border border-gray-700 focus:border-teal-500/50 rounded-lg pl-5 pr-3 py-1 text-xs font-mono font-semibold text-teal-300 outline-none transition-all"
          />
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 mt-1">
            <div className="relative">
              <input type="number" min="1" placeholder="Seconds" value={customSec}
                ...
                className="w-full bg-gray-800 border border-gray-700 focus:border-teal-500/50 rounded-lg px-2.5 pr-6 py-1 text-xs text-gray-300 placeholder-gray-600 outline-none"
              />
      ...
      <div className="flex gap-1.5">
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1 text-xs py-1.5 rounded-lg bg-teal-600 hover:bg-teal-500 disabled:opacity-40 text-white font-medium transition-colors">
    ...
    return (
      <div className="bg-gray-900 border border-gray-800 rounded-2xl px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(true)}
          className="flex items-center gap-1.5 text-xs text-teal-400 hover:text-teal-300 bg-teal-950/40 hover:bg-teal-950/60 border border-teal-900/60 rounded-lg px-2.5 py-1 transition-all">
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-800">
        ...
        <button onClick={() => setShowForm(v => !v)} className="text-xs text-teal-400 hover:text-teal-300 transition-colors">
          <Plus size={11} />
        </button>
                  ...
                  <div key={item.id}>
                    ...
                    {isEditing ? (
                      ...
                      <div className={`flex items-start gap-2 rounded-xl p-2.5 transition-colors ${
                        ...
                        item.status === 'countdown' ? 'bg-teal-950/30 border border-teal-900/40' :
                        ...
                        <div className="mt-0.5 shrink-0">
                          ...
                          {item.status === 'countdown' && <Clock size={12} className="text-teal-400" />}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {isFirst && item.status === 'queued' && (
                              <span className="text-xs text-teal-500 italic">starts first</span>
                            )}
                            ...
                            {isCountdown && remaining > 0 && (
                              <div className="flex items-center gap-1.5 flex-1">
                                ...
                                <span className="text-xs font-mono text-teal-400 tabular-nums shrink-0">{fmtCountdown(remaining)}</span>
                              </div>
                            )}
                            {isCountdown && remaining <= 0 && (
                              <span className="text-xs text-teal-400 italic">starting…</span>
                            )}
          ...
          {canRunQueue && (
            <div className="px-3 pb-3">
              <button onClick={onRunQueue}
                className="w-full flex items-center justify-center gap-2 text-xs font-semibold py-2.5 rounded-xl bg-teal-600 hover:bg-teal-500 text-white transition-all shadow-lg shadow-teal-900/30">
                <Play size={11} />
          ...
          {showForm && (
            <div className="px-4 pb-4 pt-1 border-t border-gray-800 space-y-3">
              ...
              <textarea rows={2} value={taskText} onChange={e => setTaskText(e.target.value)}
                ...
                className="w-full bg-gray-800 border border-gray-700 focus:border-teal-500/70 rounded-xl px-3 py-2 text-xs text-gray-200 placeholder-gray-600 resize-none outline-none transition-colors"
              />
              ...
              {activeItems.length > 0 && <div>
                ...
                <div className="grid grid-cols-4 gap-1">
                  {DELAY_CHIPS.map(c => (
                    <button key={c.ms}
                      onClick={() => { setDelayMs(c.ms); setCustomSec(''); setCustomMin(''); }}
                      className={`text-xs py-1.5 rounded-lg border transition-all font-mono ${delayMs === c.ms && !customSec && !customMin ? 'bg-teal-600/30 border-teal-600/60 text-teal-300' : 'bg-gray-800 border-gray-700 text-gray-500 hover:text-gray-300'}`}>
                      {c.label}
                    </button>
                  ))}
                </div>
                <div className="grid grid-cols-2 gap-1.5 mt-1.5">
                  <div className="relative">
                    <input type="number" min="1" placeholder="Seconds" value={customSec}
                      onChange={e => { setCustomSec(e.target.value); setCustomMin(''); const s = parseFloat(e.target.value); if (!isNaN(s) && s > 0) setDelayMs(Math.round(s * 1000)); }}
                      className="w-full bg-gray-800 border border-gray-700 focus:border-teal-500/50 rounded-lg px-2.5 pr-6 py-1.5 text-xs text-gray-300 placeholder-gray-600 outline-none" />
                    ...
                    return (
                      <button key={q} onClick={() => setBudgetInput(q.toFixed(2))}
                        className={`text-xs py-1.5 rounded-lg border transition-all font-mono ${
                          budgetInput === q.toFixed(2)
                            ? affordable ? 'bg-teal-600/30 border-teal-600/60 text-teal-300' : 'bg-amber-900/30 border-amber-700/60 text-amber-300'
                  ...
                  <input type="number" min={0.01} step={0.01}
                    ...
                    className={`w-full bg-gray-800 border rounded-lg pl-5 pr-3 py-1.5 text-xs font-mono font-semibold focus:outline-none transition-all ${
                      !newItemAffordable && addBudget > 0
                        ...
                        : 'border-gray-700 text-teal-300 focus:border-teal-500/50'
                    }`}
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleAdd}
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 text-xs py-2 rounded-xl bg-teal-600 hover:bg-teal-500 disabled:opacity-40 text-white font-medium transition-colors">
                  <Plus size={11} />
