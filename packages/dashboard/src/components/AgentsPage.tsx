    ...
    <div className="space-y-1">
      <div className="flex items-center gap-1.5">
        <input
          ...
          className="flex-1 bg-gray-800 border border-teal-600/50 rounded-lg px-2 py-1 text-xs text-white outline-none"
        />
...
function AgentCard({
  ...
  return (
    <div className={`bg-gray-900 border rounded-xl overflow-hidden transition-colors ${
      isMine ? 'border-teal-800/50' : 'border-gray-800'
    }`}>
      <div className="px-4 py-3">
        <div className="flex items-start gap-2.5">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
            isMine ? 'bg-teal-600/20 border border-teal-600/30' : 'bg-gray-800 border border-gray-700'
          }`}>
            <Bot size={14} className={isMine ? 'text-teal-400' : 'text-gray-500'} />
          </div>
          ...
          <div className="flex-1 min-w-0">
            {renaming ? (
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {isMine && (
                  <span className="text-xs px-1.5 py-px rounded bg-teal-950/60 border border-teal-800/60 text-teal-400 leading-none">
                    mine
                  </span>
                )}
...
const SETUP_STEPS = [
  ...
  },
  {
    ...
    body: 'Expose GET /health that returns { status: "ok" }. AgentPay pings this to verify the agent is reachable before dispatching tasks.',
  },
  {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-gray-900 border border-gray-800 rounded-xl pl-8 pr-3 py-2 text-xs text-gray-200 placeholder-gray-600 outline-none focus:border-teal-600/50 transition-colors"
          />
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {ALL_MODELS.map(m => (
            <button key={m}
              ...
              className={`text-xs px-2.5 py-1 rounded-lg border font-medium uppercase transition-all ${
                modelFilter === m
                  ? m === 'x402' ? 'bg-blue-950/60 border-blue-700 text-blue-300' : 'bg-teal-950/60 border-teal-700 text-teal-300'
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value)}
            className="text-xs bg-gray-900 border border-gray-800 rounded-lg px-2 py-1.5 text-gray-400 outline-none focus:border-teal-600/50 cursor-pointer">
      ...
      <div className="grid grid-cols-12 gap-6 items-start">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-5">
          ...
          {myAgents.length > 0 && (
            <div>
              <h3 className="text-xs text-teal-400 uppercase tracking-widest font-semibold mb-3">My Agents</h3>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-4">
          {/* Register CTA */}
          <div className="bg-gray-900 border border-teal-800/40 rounded-2xl p-4 space-y-3">
