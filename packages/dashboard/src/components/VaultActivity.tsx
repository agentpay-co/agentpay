...
export function VaultActivity({ events }: Props) {
  ...
  const barColor = pctSpent >= 90 ? 'bg-red-500' : pctSpent >= 70 ? 'bg-amber-500' : 'bg-teal-500';

  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
      {/* Header */}
      <div className="flex items-center gap-2 mb-3">
        <Shield size={14} className={state.completed ? 'text-gray-500' : 'text-teal-400'} />
        ...
        <div className="space-y-1.5 mb-3">
          {state.releases.map((r, i) => (
            <div key={i} className="flex items-center gap-2 text-xs bg-gray-800/50 rounded-lg px-2.5 py-1.5">
              <ArrowDownLeft size={10} className="text-teal-400 shrink-0" />
              <span className="text-gray-400 truncate">{r.agent_name}</span>
              <span className="text-teal-300 font-mono ml-auto shrink-0">${r.amount.toFixed(4)}</span>
              {r.tx_hash && (
                <a
